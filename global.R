...
if (file.exists("secret.yml")) {
  ...
  userData$dev <- FALSE
  for (i in 1:length(secret_pars)) userData[[names(secret_pars)[i]]] <- secret_pars[[i]]
} else {
  userData$dev <- TRUE
}
...
tabs <- list(
  ...
  QuestionTab$new("auth", "title", NULL, ui_pars = list(captcha_code=userData$captcha_code)),
